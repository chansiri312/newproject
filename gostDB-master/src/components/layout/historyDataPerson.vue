                                        ...
                                        <div class="text">
                                            <v-sheet class="form mx-auto" elevation="1" max-width="900" min-height="500"
                                                ...
                                                <div class="forminput">
                                                    ...
                                                    <v-col class="text-center" style="margin-bottom: 0rem">
                                                        <h3>แบบบันทึกข้อมูลประวัติการให้ผลผลิต(รายตัว)</h3>
                                                    </v-col>

                                                    <v-col>
                                                        <v-text-field label="กรอกข้อมูลหมายเลขแพะ" type="text"
                                                            variant="outlined" v-model="recipe.ingredient1"></v-text-field>
                                                    </v-col>

                                                    <div class="text-center" style="margin-bottom: 1rem">
                                                        <h4>ปริมาณของน้ำนม(Kg)</h4>
                                                    </div>

                                                    <v-row>
                                                        <v-col>
                                                            <v-text-field label="กรอกข้อมูลปริมาณน้ำนมช่วงเช้า" type="text"
                                                                variant="outlined"
                                                                v-model="recipe.ingredient2"></v-text-field>
                                                        </v-col>
                                                        <v-col>
                                                            <v-text-field label="กรอกข้อมูลปริมาณน้ำนมช่วงเย็น" type="text"
                                                                variant="outlined"
                                                                v-model="recipe.ingredient3"></v-text-field>
                                                        </v-col></v-row>
                                                    <v-row>
                                                        <v-col>
                                                            <v-text-field label="กรอกข้อมูลรวมปริมาณน้ำนม" type="text"
                                                                variant="outlined"
...
function showMyData() {
    if (user !== null) {
        user.providerData.forEach(async (profile) => {
            ...
            const q = query(collection(db, 'addhistoryDataPerson'), where('useremail', '==', userEmail))
