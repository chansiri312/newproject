                      ...
                      >
                        <div class="forminput">
                          <v-col class="text-center" style="margin-bottom: 1rem">
                            <h3>แบบบันทึกประวัติบุคลากร</h3>
                          </v-col>
                          <v-row>
                            <v-col>
                              <v-text-field
                                label="กรอกข้อมูลเรื่องที่ฝึกอบรม"
                                type="text"
                                variant="outlined"
                                v-model="recipe.name_train"
                              ></v-text-field>
                            </v-col>
                            <v-col>
                              <v-text-field
                                label="กรอกข้อมูลวันที่ฝึกอบรม"
                                type="text"
                                variant="outlined"
                                v-model="recipe.date_train"
                              ></v-text-field>
                            </v-col>
                          </v-row>
                          <v-row>
                            <v-col>
                              <v-text-field
                                label="กรอกข้อมูลระยะเวลาในการฝึกอบรม"
                                type="text"
                                variant="outlined"
                                v-model="recipe.time_train"
                              ></v-text-field>
                            </v-col>
                            <v-col>
                              <v-text-field
                                label="กรอกข้อมูลสถานที่ฝึกอบรม"
                                type="text"
                                variant="outlined"
                                v-model="recipe.location_train"
                              ></v-text-field>
                            </v-col>
                          </v-row>
                          <v-row>
                            <v-col>
                              <v-text-field
                                label="กรอกข้อมูลผู้จัดการฟาร์ม/ผู้ประเมินผล"
                                type="text"
                                variant="outlined"
...
function showMyData() {
  if (user !== null) {
    user.providerData.forEach(async (profile) => {
      ...
      const q = query(collection(db, 'addworkman'), where('useremail', '==', userEmail))
